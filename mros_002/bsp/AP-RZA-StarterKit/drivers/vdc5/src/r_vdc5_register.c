...
TOPPERS_SYSLOG(LOG_INFO,"VDC5_Initialize()_E");
...
TOPPERS_SYSLOG(LOG_INFO,"VDC5_Initialize()_X");
...
TOPPERS_SYSLOG(LOG_INFO,"VDC5_Terminate()_E");
    syscnt_panel_clk_reg = vdc5_regaddr_system_ctrl[ch].syscnt_panel_clk;
    /* Disable */
    syscnt_panel_clk_tmp    = (uint16_t)((uint32_t)*syscnt_panel_clk_reg & (~VDC5_REG_BIT8));
    *syscnt_panel_clk_reg   = syscnt_panel_clk_tmp;
...
TOPPERS_SYSLOG(LOG_INFO,"VDC5_Terminate()_X");
...
TOPPERS_SYSLOG(LOG_INFO,"VDC5_VideoInput()_E");
...
TOPPERS_SYSLOG(LOG_INFO,"VDC5_VideoInput()_X");
...
TOPPERS_SYSLOG(LOG_INFO,"VDC5_SyncControl()_E");
	...
	// 繧｢繝ｫ繝輔ぃ縺輔ｓ縺ｮ繧ｵ繝ｳ繝励Ν繧医ｊ蠑慕畑縲ゆｺ句燕縺ｫ螳溯｡後＠縺ｦ縺翫°縺ｪ縺・怙蠕後・ scl0_update 縺ｧ繧ｯ繝ｩ繝・す繝･縺吶ｋ 
	//vdc5_set_control_signal(VDC5_CH0, VDC5_TCON_0, VDC5_TCON_VS);
	//vdc5_set_control_signal(VDC5_CH0, VDC5_TCON_1, VDC5_TCON_HS);
	//vdc5_set_control_signal(VDC5_CH0, VDC5_TCON_6, VDC5_TCON_DE);
    output_ctrl = &vdc5_regaddr_output_ctrl[ch];
	*(output_ctrl->tcon_tim_stva2) = 0x10;
	*(output_ctrl->tcon_tim_stvb2) = 0x2;
	*(output_ctrl->tcon_tim_polb2) = 0x1007;
	*(output_ctrl->tcon_update) = 0x00000001;
	// 繧｢繝ｫ繝輔ぃ縺輔ｓ縺ｮ繧ｵ繝ｳ繝励Ν繧医ｊ蠑慕畑縺ｯ縺薙％縺ｾ縺ｧ
...
TOPPERS_SYSLOG(LOG_INFO,"VDC5_SyncControl()_X");
...
TOPPERS_SYSLOG(LOG_INFO,"VDC5_DisplayOutput()_E");
    output_ctrl = &vdc5_regaddr_output_ctrl[ch];
...
TOPPERS_SYSLOG(LOG_INFO,"VDC5_DisplayOutput()_X");
...
TOPPERS_SYSLOG(LOG_INFO,"VDC5_WriteDataControl()_E");
...
TOPPERS_SYSLOG(LOG_INFO,"VDC5_WriteDataControl()_X");
...
TOPPERS_SYSLOG(LOG_INFO,"VDC5_ChangeWriteProcess()_E");
    scaler      = &vdc5_regaddr_scaler[ch][scaling_id];
    scldw_rot   = &param->scalingdown_rot;
...
TOPPERS_SYSLOG(LOG_INFO,"VDC5_ChangeWriteProcess()_X");
...
TOPPERS_SYSLOG(LOG_INFO,"VDC5_ReadDataControl()_E");
    ...
    *(img_synthesizer->gr_flm1) |= (uint32_t)param->gr_flm_sel << VDC5_REG_SHIFT_8;
    ...
    /* Frame buffer base address */
	// ・ｽn・ｽ[・ｽh・ｽE・ｽF・ｽA・ｽﾉ設定す・ｽ・ｽt・ｽ・ｽ・ｽ[・ｽ・ｽ・ｽo・ｽb・ｽt・ｽ@・ｽﾌア・ｽh・ｽ・ｽ・ｽX・ｽ・ｽ ・ｽ・ｽ・ｽ・ｽ・ｽA・ｽh・ｽ・ｽ・ｽX・ｽﾉ変奇ｿｽ・ｽ・ｽ・ｽﾈゑｿｽ・ｽﾆゑｿｽ・ｽ・ｽ・ｽｿゃう・ｽ・ｽ・ｽH・ｽ@tokuyama@kmg
...
//	*(img_synthesizer->gr_flm2) = (uint32_t)param->gr_base & (uint32_t)~VDC5_REG_MASK_0X00000007;
	{
		...
		R_OSPL_ToPhysicalAddress(param->gr_base, (uintptr_t*)&physicalAddress);
		*(img_synthesizer->gr_flm2) = (uint32_t)physicalAddress;
//		syslog(LOG_INFO, "gr_flm2 = 0x%x", *(img_synthesizer->gr_flm2));
	}
	...
	*(img_synthesizer->gr_flm6) |= (uint32_t)param->gr_format << VDC5_REG_SHIFT_28; //tokuyama@kmg
...
TOPPERS_SYSLOG(LOG_INFO,"VDC5_ReadDataControl()_X");
...
TOPPERS_SYSLOG(LOG_INFO,"VDC5_ChangeReadProcess()_E");
...
//syslog(LOG_INFO, "gr_flm6 = 0x%x", *(img_synthesizer->gr_flm6));
		}
    }
...
//syslog(LOG_INFO, "gr_flm5 = 0x%x", *(img_synthesizer->gr_flm5));
        /* Width of the horizontal valid period */
...
TOPPERS_SYSLOG(LOG_INFO,"VDC5_ChangeReadProcess()_X");
...
TOPPERS_SYSLOG(LOG_INFO,"VDC5_StartProcess()_E");
    if (param->gr_disp_sel != NULL)
...
TOPPERS_SYSLOG(LOG_INFO,"VDC5_StartProcess()_X");
...
TOPPERS_SYSLOG(LOG_INFO,"VDC5_StopProcess()_E");
    oir_resrc_st_before = VDC5_ShrdPrmGetOirRwProcEnabled(ch);
...
TOPPERS_SYSLOG(LOG_INFO,"VDC5_StopProcess()_X");
...
TOPPERS_SYSLOG(LOG_INFO,"VDC5_ReleaseDataControl()_E");
...
TOPPERS_SYSLOG(LOG_INFO,"VDC5_ReleaseDataControl()_X");
...
TOPPERS_SYSLOG(LOG_INFO,"VDC5_VideoNoiseReduction()_E");
    input_ctrl = &vdc5_regaddr_input_ctrl[ch];
...
TOPPERS_SYSLOG(LOG_INFO,"VDC5_VideoNoiseReduction()_X");
...
{
TOPPERS_SYSLOG(LOG_INFO,"VDC5_ImageColorMatrix()_E");
    SetColorMatrix(param->mtx_mode, param->offset, param->gain, &vdc5_regaddr_color_matrix[ch][param->module]);
TOPPERS_SYSLOG(LOG_INFO,"VDC5_ImageColorMatrix()_X");
...
TOPPERS_SYSLOG(LOG_INFO,"VDC5_ImageEnhancement()_E");
...
TOPPERS_SYSLOG(LOG_INFO,"VDC5_ImageEnhancement()_X");
...
TOPPERS_SYSLOG(LOG_INFO,"VDC5_ImageBlackStretch()_E");
    img_qlty_imp = &vdc5_regaddr_img_qlty_imp[ch][imgimprv_id];
...
TOPPERS_SYSLOG(LOG_INFO,"VDC5_ImageBlackStretch()_X");
...
TOPPERS_SYSLOG(LOG_INFO,"VDC5_AlphaBlending()_E");
    img_synthesizer = &vdc5_regaddr_img_synthesizer[ch][graphics_id];
...
TOPPERS_SYSLOG(LOG_INFO,"VDC5_AlphaBlending()_X");
...
TOPPERS_SYSLOG(LOG_INFO,"VDC5_AlphaBlendingRect()_E");
    img_synthesizer = &vdc5_regaddr_img_synthesizer[ch][graphics_id];
...
TOPPERS_SYSLOG(LOG_INFO,"VDC5_AlphaBlendingRect()_X");
}   /* End of function VDC5_AlphaBlending() */
...
TOPPERS_SYSLOG(LOG_INFO,"VDC5_Chromakey()_E");
    img_synthesizer = &vdc5_regaddr_img_synthesizer[ch][graphics_id];
...
TOPPERS_SYSLOG(LOG_INFO,"VDC5_Chromakey()_X");
...
TOPPERS_SYSLOG(LOG_INFO,"VDC5_CLUT()_E");
    img_synthesizer = &vdc5_regaddr_img_synthesizer[ch][graphics_id];
...
TOPPERS_SYSLOG(LOG_INFO,"VDC5_CLUT()_X");
...
TOPPERS_SYSLOG(LOG_INFO,"VDC5_DisplayCalibration()_E");
    output_ctrl = &vdc5_regaddr_output_ctrl[ch];
...
TOPPERS_SYSLOG(LOG_INFO,"VDC5_DisplayCalibration()_X");
...
TOPPERS_SYSLOG(LOG_INFO,"VDC5_GammaCorrection()_E");
...
TOPPERS_SYSLOG(LOG_INFO,"VDC5_GammaCorrection()_X");
...
static void SetVideoExternalInput (
    ...
    const vdc5_regaddr_input_ctrl_t * const input_ctrl)
{
TOPPERS_SYSLOG(LOG_INFO,"SetVideoExternalInput()_E");
...
TOPPERS_SYSLOG(LOG_INFO,"SetVideoExternalInput()_X");
...
TOPPERS_SYSLOG(LOG_INFO,"SetInitialConnection()_E");
...
TOPPERS_SYSLOG(LOG_INFO,"SetInitialConnection()_X");
...
TOPPERS_SYSLOG(LOG_INFO,"SetGrapicsBackgroundColor()_E");
    ...
    *update_reg    |= (uint32_t)VDC5_REG_BIT4;
TOPPERS_SYSLOG(LOG_INFO,"SetGrapicsBackgroundColor()_X");
...
TOPPERS_SYSLOG(LOG_INFO,"SetScalerBackgroundColor()_E");
    if (graphics_id == VDC5_GR_TYPE_GR0)
    {
...
TOPPERS_SYSLOG(LOG_INFO,"SetScalerBackgroundColor()_X");
...
static void SetLcdTcon_STVA_VS (
    ...
    const vdc5_regaddr_output_ctrl_t    * const output_ctrl)
{
TOPPERS_SYSLOG(LOG_INFO,"SetLcdTcon_STVA_VS()_E");
...
TOPPERS_SYSLOG(LOG_INFO,"SetLcdTcon_STVA_VS()_X");
...
static void SetLcdTcon_STVB_VE (
    ...
    const vdc5_regaddr_output_ctrl_t    * const output_ctrl)
{
TOPPERS_SYSLOG(LOG_INFO,"SetLcdTcon_STVB_VE()_E");
    /* Signal pulse start position */
...
TOPPERS_SYSLOG(LOG_INFO,"SetLcdTcon_STVB_VE()_X");
...
static void SetLcdTcon_STH_HS (
    ...
    const vdc5_regaddr_output_ctrl_t    * const output_ctrl)
{
TOPPERS_SYSLOG(LOG_INFO,"SetLcdTcon_STH_HS()_E");
    /* Signal pulse start position */
...
TOPPERS_SYSLOG(LOG_INFO,"SetLcdTcon_STH_HS()_X");
...
static void SetLcdTcon_STB_HE (
    ...
    const vdc5_regaddr_output_ctrl_t    * const output_ctrl)
{
TOPPERS_SYSLOG(LOG_INFO,"SetLcdTcon_STB_HE()_E");
    /* Signal pulse start position */
...
TOPPERS_SYSLOG(LOG_INFO,"SetLcdTcon_STB_HE()_X");
...
static void SetLcdTcon_CPV_GCK (
    ...
    const vdc5_regaddr_output_ctrl_t    * const output_ctrl)
{
TOPPERS_SYSLOG(LOG_INFO,"SetLcdTcon_CPV_GCK()_E");
    /* Signal pulse start position */
...
TOPPERS_SYSLOG(LOG_INFO,"SetLcdTcon_CPV_GCK()_X");
...
static void SetLcdTcon_POLA (
    ...
    const vdc5_regaddr_output_ctrl_t    * const output_ctrl)
{
TOPPERS_SYSLOG(LOG_INFO,"SetLcdTcon_POLA()_E");
    /* Signal pulse start position */
...
TOPPERS_SYSLOG(LOG_INFO,"SetLcdTcon_POLA()_X");
...
static void SetLcdTcon_POLB (
    ...
    const vdc5_regaddr_output_ctrl_t    * const output_ctrl)
{
TOPPERS_SYSLOG(LOG_INFO,"SetLcdTcon_POLB()_E");
    /* Signal pulse start position */
...
TOPPERS_SYSLOG(LOG_INFO,"SetLcdTcon_POLB()_X");
...
TOPPERS_SYSLOG(LOG_INFO,"SetScalingDown()_E");
...
TOPPERS_SYSLOG(LOG_INFO,"SetScalingDown()_X");
...
TOPPERS_SYSLOG(LOG_INFO,"SetScalingUp()_E");
    vscl_up = VDC5_OFF;
...
TOPPERS_SYSLOG(LOG_INFO,"SetScalingUp()_X");
...
TOPPERS_SYSLOG(LOG_INFO,"CalcScalingRatio()_E");
...
TOPPERS_SYSLOG(LOG_INFO,"CalcScalingRatio()_X");
...
TOPPERS_SYSLOG(LOG_INFO,"SetInputCntrlColorMtx()_E");
...
TOPPERS_SYSLOG(LOG_INFO,"SetInputCntrlColorMtx()_X");
...
TOPPERS_SYSLOG(LOG_INFO,"ConfirmGraphicsEnlargement()_E");
...
TOPPERS_SYSLOG(LOG_INFO,"ConfirmGraphicsEnlargement()_X");
...
TOPPERS_SYSLOG(LOG_INFO,"SetScalerGraphics()_E");
    if (graphics_id == VDC5_GR_TYPE_GR0)
    {   /* Scaler 0 */
...
TOPPERS_SYSLOG(LOG_INFO,"SetScalerGraphics()_X");
...
TOPPERS_SYSLOG(LOG_INFO,"DisplayStartPixelSetting()_E");
...
TOPPERS_SYSLOG(LOG_INFO,"DisplayStartPixelSetting()_X");
    return gr_sta_pos;
...
TOPPERS_SYSLOG(LOG_INFO,"SetCascade()_E");
...
TOPPERS_SYSLOG(LOG_INFO,"SetCascade()_X");
    return cascade;
...
TOPPERS_SYSLOG(LOG_INFO,"SetupGraphicsVIN()_E");
    /* Cascade */
...
TOPPERS_SYSLOG(LOG_INFO,"SetupGraphicsVIN()_X");
...
TOPPERS_SYSLOG(LOG_INFO,"ChangeGraphicsVIN()_E");
    /* Cascade */
...
TOPPERS_SYSLOG(LOG_INFO,"ChangeGraphicsVIN()_X");
...
TOPPERS_SYSLOG(LOG_INFO,"SetImgQaImproverColorMtx()_E");
    /* Color matrix operating mode */
    if (color_space == VDC5_COLOR_SPACE_GBR)
    {
        mtx_mode_tmp = VDC5_COLORMTX_GBR_GBR;
...
TOPPERS_SYSLOG(LOG_INFO,"SetImgQaImproverColorMtx()_X");
...
TOPPERS_SYSLOG(LOG_INFO,"SetRwProcAbility_Write_0()_E");
    /* SC0/GR0 writing */
    scaler = &vdc5_regaddr_scaler[ch][VDC5_SC_TYPE_SC0];
...
TOPPERS_SYSLOG(LOG_INFO,"SetRwProcAbility_Write_0()_X");
...
TOPPERS_SYSLOG(LOG_INFO,"SetRwProcAbility_Write_1()_E");
    /* SC1/GR1 writing */
    scaler = &vdc5_regaddr_scaler[ch][VDC5_SC_TYPE_SC1];
...
TOPPERS_SYSLOG(LOG_INFO,"SetRwProcAbility_Write_1()_X");
...
TOPPERS_SYSLOG(LOG_INFO,"SetRwProcAbility_Write_OIR()_E");
    /* OIR writing */
    scaler = &vdc5_regaddr_scaler[ch][VDC5_SC_TYPE_OIR];
...
TOPPERS_SYSLOG(LOG_INFO,"SetRwProcAbility_Write_OIR()_X");
...
TOPPERS_SYSLOG(LOG_INFO,"SetRwProcAbility_Read_0()_E");
    /* SC0/GR0 reading */
    img_synthesizer = &vdc5_regaddr_img_synthesizer[ch][VDC5_GR_TYPE_GR0];
...
TOPPERS_SYSLOG(LOG_INFO,"SetRwProcAbility_Read_0()_X");
...
TOPPERS_SYSLOG(LOG_INFO,"SetRwProcAbility_Read_1()_E");
    /* SC1/GR1 reading */
    img_synthesizer = &vdc5_regaddr_img_synthesizer[ch][VDC5_GR_TYPE_GR1];
...
TOPPERS_SYSLOG(LOG_INFO,"SetRwProcAbility_Read_1()_X");
...
TOPPERS_SYSLOG(LOG_INFO,"SetRwProcAbility_Read_2()_E");
    /* GR2 reading */
...
TOPPERS_SYSLOG(LOG_INFO,"SetRwProcAbility_Read_2()_X");
...
TOPPERS_SYSLOG(LOG_INFO,"SetRwProcAbility_Read_3()_E");
    /* GR3 reading */
...
TOPPERS_SYSLOG(LOG_INFO,"SetRwProcAbility_Read_3()_X");
...
TOPPERS_SYSLOG(LOG_INFO,"SetRwProcAbility_Read_OIR()_E");
    /* OIR reading */
...
TOPPERS_SYSLOG(LOG_INFO,"SetRwProcAbility_Read_OIR()_X");
...
TOPPERS_SYSLOG(LOG_INFO,"SetRegUpdateRwEnable()_E");
    switch (layer_id)
    {
...
TOPPERS_SYSLOG(LOG_INFO,"SetRegUpdateRwEnable()_X");
...
TOPPERS_SYSLOG(LOG_INFO,"SetGraphicsDisplayMode()_E");
    if (layer_id == VDC5_LAYER_ID_ALL)
    {
...
TOPPERS_SYSLOG(LOG_INFO,"SetGraphicsDisplayMode()_X");
...
static void SetColorMatrix (
    ...
    const vdc5_regaddr_color_matrix_t   * const color_matrix)
{
TOPPERS_SYSLOG(LOG_INFO,"SetColorMatrix()_E");
    /* Operating mode */
...
TOPPERS_SYSLOG(LOG_INFO,"SetColorMatrix()_X");
...
TOPPERS_SYSLOG(LOG_INFO,"SetImageEnhancementSharpness()_E");
    if (sharp_param != NULL)
    {
...
TOPPERS_SYSLOG(LOG_INFO,"SetImageEnhancementSharpness()_X");
...
TOPPERS_SYSLOG(LOG_INFO,"SetImageEnhancementLti()_E");
    if (lti_param != NULL)
    {
        /* H2, second adjacent pixel used as reference */
...
TOPPERS_SYSLOG(LOG_INFO,"SetImageEnhancementLti()_X");
...
TOPPERS_SYSLOG(LOG_INFO,"SetAlphaRectArea()_E");
...
TOPPERS_SYSLOG(LOG_INFO,"SetAlphaRectArea()_X");
...
TOPPERS_SYSLOG(LOG_INFO,"SetUndSel()_E");
...
TOPPERS_SYSLOG(LOG_INFO,"SetUndSel()_X");
...
TOPPERS_SYSLOG(LOG_INFO,"ColorConversion()_E");
    conv_color = 0;
    switch (gr_format)
    {
...
TOPPERS_SYSLOG(LOG_INFO,"ColorConversion()_X");
    return conv_color;
...
TOPPERS_SYSLOG(LOG_INFO,"ColorConversionFrom4to8()_E");
    color = color_value & (uint32_t)VDC5_REG_COLOR_4BIT_MASK;
    color *= (uint32_t)VDC5_REG_EXTENSION_VALUE_17;

TOPPERS_SYSLOG(LOG_INFO,"ColorConversionFrom4to8()_X");
    return color;
...
TOPPERS_SYSLOG(LOG_INFO,"ColorConversionFrom5to8()_E");
...
TOPPERS_SYSLOG(LOG_INFO,"ColorConversionFrom5to8()_X");
    return color;
...
TOPPERS_SYSLOG(LOG_INFO,"ColorConversionFrom6to8()_E");
...
TOPPERS_SYSLOG(LOG_INFO,"ColorConversionFrom6to8()_X");
    return color;
...
TOPPERS_SYSLOG(LOG_INFO,"ColorConversionIntoAgbr()_E");
    /* Green and blue */
...
TOPPERS_SYSLOG(LOG_INFO,"ColorConversionIntoAgbr()_X");
    return color_bgr;
...
TOPPERS_SYSLOG(LOG_INFO,"Set_Clut()_E");
    clut_tmp = param->clut;
...
TOPPERS_SYSLOG(LOG_INFO,"Set_Clut()_X");
...
TOPPERS_SYSLOG(LOG_INFO,"Set_StartThreshold_Gamma()_E");
    if (gam_th != NULL)
    {
...
TOPPERS_SYSLOG(LOG_INFO,"Set_StartThreshold_Gamma()_X");
...
TOPPERS_SYSLOG(LOG_INFO,"Set_GainAdjustment_Gamma()_E");
    if (gam_gain != NULL)
    {
...
TOPPERS_SYSLOG(LOG_INFO,"Set_GainAdjustment_Gamma()_X");
