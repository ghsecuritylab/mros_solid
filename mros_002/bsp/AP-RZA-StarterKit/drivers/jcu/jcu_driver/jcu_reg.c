...
__DMB();
	...
	R_SET_REG_BIT_FIELD(JCU.JCMOD, DSP, codec); // リセット直後にやった方が良いのではと思って移動。tokuyama@kmg 2016.09.20

	/* Enable interrupts in registers (Interrupt lines in INTC are still disabled) */
    /* This registers are set to 0 by reset this IP */
    /* ->QAC 0306 */
//	JCU.JINTE1 = (uint32_t)JCU_JINTS1_ALL; //DBTFを disable にしておくとよくないことが起きる... tokuyama@kmg
	JCU.JINTE1 = (uint32_t)0;
    /* <-QAC 0306 */
    ...
    void)
{
	...
	R_SET_REG_BIT_FIELD(JCU.JCCMD, JEND, REG_F_SET); // 順番が逆だったかもしれない tokuyama@kmg
...
void JCU_SetRegisterForSetPause( const bool_t is_pause, const jcu_int_detail_errors_t filter )
{
    ...
    if ( IS( is_pause ) )
    {
        JCU.JINTE0 = (uint8_t)( filter | INT3_MASK );
    }
    else
    {
        JCU.JINTE0 = (uint8_t) filter;
    }
    /* <-QAC 0306 */
}
...
errnum_t  R_JCU_OnInterrupting( const r_ospl_interrupt_t* const  InterruptSource )
{
    ...
    ASSERT_R( self->AsyncStatus.Status == JCU_STATUS_INTERRUPTING,  e=E_STATE; goto skip );
	...
	/* Interrupt request clear */
	R_SET_REG_BIT_FIELD(JCU.JCCMD, JEND, REG_F_SET); // これを忘れていると DBTF 割り込みがねげーとされない
